	...
	const missing = (
		[
			['PUBLIC_API_URL', import.meta.env.PUBLIC_API_URL],
				['INTERNAL_API_URL', import.meta.env.INTERNAL_API_URL],
...
const PUBLIC_API_URL =
	import.meta.env.PUBLIC_API_URL || 'https://api.imanjo.com/api';

const INTERNAL_API_URL =
	import.meta.env.INTERNAL_API_URL || PUBLIC_API_URL;
...
export async function apiFetch<T = unknown>(path: string, options: ApiFetchOptions = {}): Promise<ApiResult<T>> {
	...
	try {
		...
		try {
			...
			console.error(
				`[apiFetch] ${path} returned invalid JSON (HTTP ${res.status})`,
				parseError
			);
			...
			return {
				ok: false,
				data: null,
				message: 'استجابة غير صالحة من الخادم، يرجى المحاولة لاحقًا.',
				meta: null,
		...
		if (!json || typeof json !== 'object' || Array.isArray(json)) {
			console.error(
				`[apiFetch] ${path} returned an invalid API envelope (HTTP ${res.status})`
			);
