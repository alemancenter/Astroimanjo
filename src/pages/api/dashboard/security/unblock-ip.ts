...
export const POST: APIRoute = async ({ request, cookies, locals, redirect }) => {
	...
	const res = await apiRawFetch(`/dashboard/security/blocked-ips/${encodeURIComponent(ip)}`, {
		method: 'DELETE',
	...
	if (!res.ok || json?.success === false) {
		return redirect(`${redirectTo}${separator}error=${encodeURIComponent(json?.message || 'فشل إلغاء الحظر')}`);
	}
