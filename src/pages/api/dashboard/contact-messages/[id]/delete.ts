...
export const POST: APIRoute = async ({ params, request, cookies, locals, redirect }) => {
	...
	const res = await apiRawFetch(`/dashboard/contact-messages/${params.id}`, {
		method: 'DELETE',
	...
	if (!res.ok || json?.success === false) {
		return redirect(`${redirectTo}${separator}error=${encodeURIComponent(json?.message || 'فشل حذف الرسالة')}`);
	}
