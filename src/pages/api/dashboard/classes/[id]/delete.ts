...
export const POST: APIRoute = async ({ params, request, cookies, locals, redirect, cache }) => {
	...
	const res = await apiRawFetch(`/dashboard/school-classes/${params.id}`, {
		method: 'DELETE',
	...
	if (!res.ok || json?.success === false) {
		return redirect(addNotice(redirectTo, 'error', json?.message || 'فشل حذف الصف'));
	}
	await cache.invalidate({ tags: ['classes', 'subjects', 'articles', 'files'] });
	return redirect(addNotice(redirectTo, 'success', 'تم حذف الصف الدراسي'));
};
