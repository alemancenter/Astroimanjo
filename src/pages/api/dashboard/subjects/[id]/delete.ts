...
export const POST: APIRoute = async ({ params, request, cookies, locals, redirect, cache }) => {
	...
	const res = await apiRawFetch(`/dashboard/subjects/${params.id}`, {
		method: 'DELETE',
	...
	if (!res.ok || json?.success === false) {
		return redirect(addNotice(redirectTo, 'error', json?.message || 'فشل حذف المادة'));
	}
	await cache.invalidate({ tags: ['subjects', 'classes'] });
	return redirect(addNotice(redirectTo, 'success', 'تم حذف المادة'));
};
